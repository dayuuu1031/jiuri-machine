...
const App: React.FC = () => {
  ...
  const [currentProductTab, setCurrentProductTab] = useState<string>('core');
  
  const [isChatOpen, setIsChatOpen] = useState(false);

  const navigate = (view: Section, tab?: any) => {
    setCurrentView(view);
    if (view === Section.ABOUT && tab) {
      setCurrentAboutTab(tab);
    }
    if (view === Section.PRODUCTS && tab) {
    ...
    <div className="min-h-screen bg-white font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="transition-opacity duration-300">
        {currentView === Section.HOME && (
          <HomeView 
            lang={lang} 
            onExplore={() => navigate(Section.PRODUCTS)} 
        ...
        {currentView === Section.PRODUCTS && (
          <ProductsView 
            lang={lang} 
            activeTab={currentProductTab} 
            onConsult={() => setIsChatOpen(true)}
          />
        )}

        {currentView === Section.ADVANTAGES && (
          <AdvantagesView lang={lang} onConsult={() => setIsChatOpen(true)} />
        )}
      ...
      <AIChat isOpen={isChatOpen} onToggle={setIsChatOpen} />
    </div>
  );
};
