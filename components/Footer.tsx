...
interface FooterProps {
  lang: 'zh' | 'en';
  // Fixed: Updated onNavigate signature to accept an optional tab argument to match App.tsx and its usage in the component
...
const Footer: React.FC<FooterProps> = ({ lang, onNavigate }) => {
  ...
  const tf = (TRANSLATIONS[lang] as any).footer || { desc: '', products_list: [] };

  return (
    <footer className="bg-gray-900 text-white pt-24 pb-12 border-t-8 border-[#004ea1]">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          <div>
            <div className="flex items-center space-x-3 mb-8">
               <img src="https://i.postimg.cc/PfFjfq6G/jiang-su-jiu-rilogo-Picsart-Background-Remover.png" className="h-10 w-auto rounded-none bg-white p-0.5" alt="JIURI" />
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 text-sm font-medium">
              {tf.products_list.map((pName: string) => (
                <li key={pName}>
                  <button 
                    onClick={() => onNavigate(Section.PRODUCTS)} 
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 text-sm font-medium">
              {/* Fixed: These calls now match the updated onNavigate signature */}
              <li><button onClick={() => onNavigate(Section.ABOUT, 'overview')} className="hover:text-blue-500 transition-colors">{t.about_overview}</button></li>
              <li><button onClick={() => onNavigate(Section.ABOUT, 'history')} className="hover:text-blue-500 transition-colors">{t.about_history}</button></li>
              <li><button onClick={() => onNavigate(Section.ABOUT, 'honors')} className="hover:text-blue-500 transition-colors">{t.about_honors}</button></li>
            </ul>
          </div>
